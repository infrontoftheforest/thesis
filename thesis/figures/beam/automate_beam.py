    ...
    if args.plot:
        ...
        for precond in preconditioners:
            for ratio in ratios:
                if len(preconditioners) > 1:
                    ax.plot(elems[precond][ratio], lin_solve_time[precond][ratio], marker='o', label=precond_to_str[precond], markersize=3)
                else:
                    ax.plot(elems[precond][ratio], lin_solve_time[precond][ratio], marker='o', label=f'Height ratio $r$ = {ratio}', markersize=3)
        if len(ratios) > 1 or len(preconditioners) > 1:
            ax.legend()
        ...
        if len(preconditioners) > 1:
            ...
            if preconditioner == 'muelu':
                ...
                title_str = 'SuperLU'
        # ax.set_title(f'{title_str} (~10,000 elements/processor)')
        ax.set_xlim(left=0)
